...
def play_game():
    state_space = (4, 4, 5, 3, 3, 3, 3)  # Example state space
    action_space = 3  # Number of possible actions (jump, crouch, stand)
    agent = QLearningAgent(state_space, action_space)

    # Load the trained Q-table
    agent.load_model('q_learning_model.pkl')  # Replace with your file name
    ...
    for episode in range(episodes):
        ...
        reward = 0  # Reset reward for each episode
        running = True
        
        while running:
            screen.fill(BLUE)  # Set background to blue
            ...
            # Choose action based on the current state (no exploration here)
            ...
            if collision:
                print(f"Game Over! Episode {episode + 1} Total Reward: {reward}")
                running = False
